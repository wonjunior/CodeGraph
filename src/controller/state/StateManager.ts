...
import { StateData, State } from './interfaces'
...
export class StateManager {
     ...
     */
    constructor(id: symbol, { keybinds, mousebinds }: State) {
     ...
     */
    static change(id: symbol, data: StateData = {}) {
        assert(StateManager.all.has(id), `The provided id (${String(id)}) doesn't exist`)

        // $.State.log(`state changed to ${State.all.get(symbol).id.toString()}`)
        StateManager.current = { ...(StateManager.all.get(id) as State), data }
    }
